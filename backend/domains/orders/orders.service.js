...
import {
    ...
    trackPurchase,
} from "../../infra/utils/google analytics/events.js";
import {
...
export async function confirmOrderPaymentService(reference, req) {
    ...
    if (
        ...
        logOrderPaymentConfirmed(orderToConfirm._id, orderToConfirm.user_id);
...
export async function revalidateOrderPaymentService(orderId, userData) {
    ...
    logOrderRevalidation(duplicateOrder._id, duplicateOrder.user_id);
...
export async function cancelOrderService(orderId, req) {
    ...
    logOrderCancellation(orderToCancel._id, orderToCancel.user_id);
}
...
export async function getAllOrdersService(userId, limit, page) {
    ...
    return {
        ...
        orders: await Promise.all(
            orders.map((order) => order.getOrderDetails()),
        ),
    };
}
